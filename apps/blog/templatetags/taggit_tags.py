...
#from mcblog.utils import LINEAR, LOGARITHMIC, calculate_cloud
import pdb
register = Library()
...
class TagsForModelNode(Node):
    ...
    def render(self, context):
        ...
        context[self.context_var] = model.tags.all() 
...
class TagCloudForModelNode(Node):
    ...
    def render(self, context):
        ...
        context[self.context_var] = calculate_cloud(model.tags.most_common().order_by('name'))
        return ''
...
class TaggedObjectsNode(Node):
    ...
    def render(self, context):
        ...
        context[self.context_var] = model.objects.filter(tags__in=[self.tag.resolve(context)])
        return ''

class RelatedTagsNode(Node):
...
def related_tags(parser, token):
    ...
    if bits[2] != 'as':
        raise TemplateSyntaxError(_("second argument to %s tag must be 'as'") % bits[0])
    #pdb.set_trace()
    return RelatedTagsNode(bits[1], bits[3])
